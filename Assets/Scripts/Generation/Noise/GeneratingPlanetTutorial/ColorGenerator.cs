    ...
    public void UpdateColors()
    {
        ...
        foreach (var biome in settings.biomeColorSettings.biomes)
        {
            for (int i = 0; i < textureResolution * 2; i++)
            {
                ...
                {
                    gradientCol = biome.gradientUnderWater.Evaluate(i / (textureResolution - 1f));

                }
                else
                {
                    gradientCol = biome.gradientAboveWater.Evaluate((i - textureResolution) / (textureResolution - 1f));
                }
